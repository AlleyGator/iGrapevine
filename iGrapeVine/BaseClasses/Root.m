...
+ (FileFormatType)detectFileFormat:(NSString *)fileNamePath {
    ...
    NSFileHandle *fileHandle = [NSFileHandle fileHandleForReadingAtPath:fileNamePath];
    fileNum = [fileHandle fileDescriptor];
    
    int i = [self getIntB:fileNum];
    NSString *binaryHeader = [self getStrB:fileNum];
    NSString *sHeader = [self getStrB:fileNum];
    
    NSDictionary *map = @{BinHeaderGame: @(gvBinaryGame),
                          BinHeaderMenu: @(gvBinaryMenu),
                          BinHeaderExchange: @(gvBinaryExchange)};
    NSNumber *value = [map objectForKey:binaryHeader];
    if (!value) {
        if ([sHeader hasPrefix:@"<?xml"]) {
            value = @(gvXML);
        }
        else {
            value = @(gvInvalid); // Version 2.3 Unsupported
        }
    }
    i = 0;
...
+ (BOOL)outsideQuotes:(NSString *)quoteString location:(long)location {
    // VB Locations start at 1.
    location = location - 1;
    assert(location >= 0 && @"Location should never be negative");
    NSScanner *scanner = [NSScanner scannerWithString:quoteString];
    NSString *scanString;
    BOOL outsideQuotes = YES;
    while (![scanner isAtEnd] && [scanner scanLocation] < location) {
        // Skip non " characters
        [scanner scanUpToString:@"""" intoString:NULL];
        // If we found a ", and we haven't hit the stop location, we reverse the OutsideQuotes bool.
...
+ (void)putStrB:(int)fileNum stringValue:(NSString *)string {
    NSFileHandle *fileHandle = [[NSFileHandle alloc] initWithFileDescriptor:fileNum];
    int16_t length;
    if (string.length > INT16_MAX)
        length = INT16_MAX;
    else
    ...
    [fileHandle writeData:[[string substringWithRange:NSMakeRange(0, length)] dataUsingEncoding:NSUTF8StringEncoding]];
}
//
...
//    
//End Sub
static const int vbBytesToAnInt = 2;

+ (void)getStrB:(int)fileNum stringValue:(NSString *__autoreleasing *)string {
    NSFileHandle *fileHandle = [[NSFileHandle alloc] initWithFileDescriptor:fileNum];
    int length = [self getIntB:fileNum];
    NSData *data = [fileHandle readDataOfLength:length];
    *string = [[NSString alloc] initWithBytes:data.bytes length:length encoding:NSUTF8StringEncoding];
}
@end

@implementation Root (NonGrapevine)

+ (int)getIntB:(int)fileNum {
    NSFileHandle *fileHandle = [[NSFileHandle alloc] initWithFileDescriptor:fileNum];
    NSMutableData *allData = [[NSMutableData alloc] initWithCapacity:2];
    NSData *data = [fileHandle readDataOfLength:2];
    int16_t charData;
    while (data.length) {
        [allData appendData:data];
        [data getBytes:&charData length:2];
        break;
    }
    int16_t intBinary = 0;
    [allData getBytes:&intBinary length:allData.length];
    
    return intBinary;
}

+ (void)putIntB:(int)fileNum integer:(int16_t)i {
    NSFileHandle *fileHandle = [[NSFileHandle alloc] initWithFileDescriptor:fileNum];
...
+ (NSString *)getStrB:(int)fileNum {
    ...
    [self getStrB:fileNum stringValue:&output];
    return output;
}
...
+ (double)getDoubleB:(int)fileNum {
    ...
    double doubleData;
    [data getBytes:&doubleData length:8];
    
    return doubleData;
}

+ (void *)getObjectBytes:(int)fileNum {
    void *bytes;
    NSFileHandle *fileHandle = [[NSFileHandle alloc] initWithFileDescriptor:fileNum];
    int length = [self getIntB:fileNum];
    NSData *data = [fileHandle readDataOfLength:length];
